...
$('#authForm').addEventListener('submit', async (event) => { event.preventDefault(); const form = new FormData(event.currentTarget); try { const result = await request(state.isLogin ? '/api/auth/login' : '/api/auth/register', { method: 'POST', body: JSON.stringify({ email: form.get('email'), password: form.get('password'), displayName: form.get('displayName') }) }); state.user = result.user; const me = await request('/api/me'); state.preferences = me.preferences; $('#authDialog').close(); if (location.pathname.startsWith('/article/')) return renderArticlePage(location.pathname.split('/')[2]); if (location.pathname.startsWith('/post/')) return renderPostPage(location.pathname.split('/')[2]); if (location.pathname === '/write') return renderWritePage(); renderProfile(); openSettings(); loadFeed(); } catch (error) { $('#authMessage').textContent = error.message; } });
...
function commentMarkup(comment, depth = 0) { return `<div class="comment-thread" style="--reply-depth:${Math.min(depth, 4)}"><article class="comment"><div class="comment-avatar">${escapeHtml(comment.author.slice(0, 1).toUpperCase())}</div><div><div class="comment-meta"><strong>${escapeHtml(comment.author)}</strong><span>${relativeDate(comment.createdAt)}</span></div><p>${escapeHtml(comment.body)}</p><button class="reply-button" data-reply-id="${comment.id}" data-reply-author="${escapeHtml(comment.author)}">Ответить</button></div></article>${comment.replies.length ? `<div class="comment-children">${comment.replies.map((reply) => commentMarkup(reply, depth + 1)).join('')}</div>` : ''}</div>`; }
async function loadComments(articleId) { const { comments } = await request(`/api/articles/${articleId}/comments`); $('#commentsList').innerHTML = comments.length ? comments.map((comment) => commentMarkup(comment)).join('') : '<p class="comments-empty">Пока нет комментариев. Начните обсуждение.</p>'; }
...
async function renderPostPage(postId) {
  ...
  $('main').innerHTML = `<article class="reader user-post"><a class="reader-back" href="/">← Вернуться к ленте</a><div class="reader-meta"><span class="tag tools">Авторская статья</span><span>${escapeHtml(post.author)}</span><span>${relativeDate(post.publishedAt)}</span></div><h1>${escapeHtml(post.title)}</h1><div class="reader-content post-content" id="postBlocks">${post.blocks.map((block) => postBlockMarkup(block, post)).join('')}</div></article>`;
...
async function init() { try { const [me, sources] = await Promise.all([request('/api/me'), request('/api/sources')]); state.user = me.user; state.preferences = me.preferences || state.preferences; state.sources = sources.sources; if (location.pathname.startsWith('/article/')) return renderArticlePage(location.pathname.split('/')[2]); if (location.pathname.startsWith('/post/')) return renderPostPage(location.pathname.split('/')[2]); if (location.pathname === '/write') return renderWritePage(); renderSources(); renderProfile(); await loadFeed(); } catch (error) { const target = $('#articleList') || $('main'); target.innerHTML = `<div class="empty-state"><strong>Не удалось загрузить материал.</strong><span>${escapeHtml(error.message)}</span></div>`; } }
init();
