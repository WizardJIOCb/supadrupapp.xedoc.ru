...
async function init() { try { const [me, sources] = await Promise.all([request('/api/me'), request('/api/sources')]); state.user = me.user; state.preferences = me.preferences || state.preferences; state.sources = sources.sources; syncTopicControls(); renderProfile(); if (location.pathname.startsWith('/article/')) return renderArticlePage(location.pathname.split('/')[2]); if (location.pathname.startsWith('/post/')) return renderPostPage(location.pathname.split('/')[2]); if (location.pathname.startsWith('/profile/')) return renderProfilePage(location.pathname.split('/')[2]); if (location.pathname === '/write') return renderWritePage(); if (location.pathname === '/about') return renderAboutPage(); renderSources(); await loadFeed(); } catch (error) { const target = $('#articleList') || $('main'); target.innerHTML = `<div class="empty-state"><strong>Не удалось загрузить материал.</strong><span>${escapeHtml(error.message)}</span></div>`; } }
init();
