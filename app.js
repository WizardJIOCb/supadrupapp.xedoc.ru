...
function commentMarkup(comment) { return `<article class="comment"><div class="comment-avatar">${escapeHtml(comment.author.slice(0, 1).toUpperCase())}</div><div><div class="comment-meta"><strong>${escapeHtml(comment.author)}</strong><span>${relativeDate(comment.createdAt)}</span></div><p>${escapeHtml(comment.body)}</p></div></article>`; }
async function loadComments(articleId) { const { comments } = await request(`/api/articles/${articleId}/comments`); $('#commentsList').innerHTML = comments.length ? comments.map(commentMarkup).join('') : '<p class="comments-empty">Пока нет комментариев. Начните обсуждение.</p>'; }
async function renderArticlePage(articleId) {
  const { article } = await request(`/api/articles/${articleId}`);
  const paragraphs = article.content.split(/\n\n+/).filter(Boolean).map((paragraph) => `<p>${escapeHtml(paragraph)}</p>`).join('');
  const composer = state.user ? `<form class="comment-form" id="commentForm"><textarea name="body" maxlength="1500" required minlength="2" placeholder="Поделитесь мыслью о материале…"></textarea><div><span>От имени ${escapeHtml(state.user.displayName || state.user.email.split('@')[0])}</span><button>Отправить</button></div></form>` : `<div class="comment-login"><span>Хотите обсудить статью?</span><button id="commentLogin">Войти или создать аккаунт</button></div>`;
  ...
  $('#commentForm')?.addEventListener('submit', async (event) => { event.preventDefault(); const textarea = event.currentTarget.elements.body; const button = event.currentTarget.querySelector('button'); button.disabled = true; try { const { comments } = await request(`/api/articles/${articleId}/comments`, { method: 'POST', body: JSON.stringify({ body: textarea.value }) }); textarea.value = ''; $('#commentsList').innerHTML = comments.map(commentMarkup).join(''); } catch (error) { textarea.setCustomValidity(error.message); textarea.reportValidity(); textarea.setCustomValidity(''); } finally { button.disabled = false; } });
