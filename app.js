...
function articleMarkup(article) {
  return `<article class="article-card"><a href="${escapeHtml(article.url)}" target="_blank" rel="noopener"><div class="article-top"><span class="tag ${article.category}">${labels[article.category] || 'News'}</span><span class="source-dot" style="--accent:${escapeHtml(article.accent)}"></span><span>${escapeHtml(article.sourceName)}</span></div><h3>${escapeHtml(article.title)}</h3><p>${escapeHtml(article.summary || 'Открыть публикацию в источнике.')}</p><div class="article-footer"><span>${relativeDate(article.publishedAt)}</span><span>Открыть ↗</span></div></a></article>`;
}
