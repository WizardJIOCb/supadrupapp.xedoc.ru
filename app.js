...
async function request(path, options = {}) {
  if (path === '/api/posts' && options.method === 'POST' && typeof options.body === 'string') {
    const post = JSON.parse(options.body);
    post.category = window.supaWriterDraft?.category || 'tools';
    options = { ...options, body: JSON.stringify(post) };
  }
...
function articleMarkup(article) {
  ...
  const category = article.category || 'tools';
...
async function loadFeed() {
  ...
  if (isHighlights) {
    ...
    if (!state.sourceIds.length) {
      ...
      ownPosts = posts.filter((post) => state.topic === 'all' || post.category === state.topic).map((post) => ({ ...post, summary: post.blocks.find((block) => block.type === 'paragraph' || block.type === 'quote')?.text || 'Авторская публикация сообщества.' }));
    }
  }
...
function searchResultMarkup(item) {
  ...
  const kind = isPost ? (labels[item.category] || 'Статья') : (labels[item.category] || 'Материал');
...
document.addEventListener('click', (event) => {
  ...
  if (window.supaWriterDraft) window.supaWriterDraft.category = category.dataset.category;
});
...
function profilePostsMarkup(posts) {
  ...
  return `<div class="profile-posts">${posts.map((post) => `<a class="profile-post" href="/post/${post.id}"><span class="tag ${post.category || 'tools'}">${escapeHtml(labels[post.category] || 'Статья')}</span><h3>${escapeHtml(post.title)}</h3><p>${escapeHtml(post.blocks.find((block) => block.type === 'paragraph' || block.type === 'quote')?.text || 'Авторская публикация.')}</p><small>${relativeDate(post.publishedAt)} · Читать →</small></a>`).join('')}</div>`;
}
...
async function renderWritePage() {
  ...
  const draft = { title: '', category: 'tools', blocks: [blankBlock('paragraph')] };
  window.supaWriterDraft = draft;
  ...
  const draw = () => { $('main').innerHTML = `<section class="writer"><a class="reader-back" href="/">← К ленте</a><div class="writer-heading"><span class="tag ${draft.category}">Новая публикация</span><h1>Напишите то, что<br />стоит прочитать.</h1><input id="postTitle" value="${escapeHtml(draft.title)}" placeholder="Заголовок статьи" /><section class="writer-category" aria-label="Тема статьи"><span>Тема статьи</span><div>${topicChoices.map(([key, name]) => `<button type="button" class="${draft.category === key ? 'active' : ''}" data-category="${key}">${name}</button>`).join('')}</div></section></div><div class="editor-toolbar"><button data-add="paragraph">Текст</button><button data-add="heading">Подзаголовок</button><button data-add="quote">Цитата</button><button data-add="image">Картинка</button><button data-add="poll">Опрос</button><button data-add="divider">Разделитель</button></div><div class="editor-blocks" id="editorBlocks">${draft.blocks.map(editorBlock).join('')}</div><div class="publish-bar"><span>Публикация сразу появится в общей ленте.</span><button id="publishPost">Опубликовать →</button></div></section>`; bind(); };
