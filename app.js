...
async function renderArticlePage(articleId) {
  ...
  $('#cancelReply')?.addEventListener('click', (event) => { event.preventDefault(); event.stopPropagation(); clearReply(); });
  $('#commentForm')?.addEventListener('click', (event) => { if (event.target.closest('#cancelReply')) { event.preventDefault(); clearReply(); } });
  $('#commentsList').addEventListener('click', async (event) => { const remove = event.target.closest('[data-delete-comment]'); if (remove) { if (!confirm('Удалить комментарий и все ответы на него?')) return; remove.disabled = true; try { await request(`/api/articles/${articleId}/comments/${remove.dataset.deleteComment}`, { method: 'DELETE' }); if (replyTo?.id === Number(remove.dataset.deleteComment)) clearReply(); await loadComments('articles', articleId); } catch (error) { alert(error.message); remove.disabled = false; } return; } const button = event.target.closest('.reply-button'); if (!button) return; if (!state.user) return openAuth(true); replyTo = { id: Number(button.dataset.replyId), author: button.dataset.replyAuthor }; $('#commentReplyLabel').textContent = `Ответ для ${replyTo.author}`; $('#commentReplyTarget').hidden = false; $('#commentForm textarea').focus(); });
...
async function renderAdminPage(range = adminRange(30), activeTab = 'stats') {
  ...
  const [statsResult, usersResult, contentResult] = await Promise.all([request(`/api/admin/stats?${query}`), request('/api/admin/users'), request('/api/admin/content')]);
  const { totals, series } = statsResult;
  const tabs = [['stats', 'Статистика'], ['users', `Пользователи · ${usersResult.users.length}`], ['content', `Контент · ${contentResult.content.length}`]];
  const userRows = usersResult.users.map((item) => `<tr><td><strong>${escapeHtml(item.displayName)}</strong><small>${escapeHtml(item.email)}</small></td><td>${new Date(item.createdAt).toLocaleDateString('ru-RU')}</td><td>${item.postCount} / ${item.commentCount}</td><td>${item.isAdmin ? '<span class="admin-status admin-status-violet">админ</span>' : ''}${item.isBanned ? '<span class="admin-status admin-status-red">заблокирован</span>' : ''}</td><td><button class="admin-action" data-admin-user="${item.id}" data-admin-field="isBanned" data-admin-value="${!item.isBanned}">${item.isBanned ? 'Разблокировать' : 'Заблокировать'}</button>${item.id !== state.user.id ? `<button class="admin-action" data-admin-user="${item.id}" data-admin-field="isAdmin" data-admin-value="${!item.isAdmin}">${item.isAdmin ? 'Снять админа' : 'Сделать админом'}</button>` : ''}</td></tr>`).join('');
